...
import { PenyusunKredensial, KredensialData } from '@/komponen/fitur/Kredensial/PenyusunKredensial';
...
export default function NoteEditorPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const [kredensial, setKredensial] = useState<KredensialData>({ username: '', password: '', url: '' });
    const [showSuggestions, setShowSuggestions] = useState(false);
    const [showOptions, setShowOptions] = useState(false);

    // Use debounced values for autosave
        ...
        const loadNote = async () => {
            try {
                ...
                if (data) {
                    ...
                    if (data.kredensial) {
                         const creds = typeof data.kredensial === 'string' ? JSON.parse(data.kredensial) : data.kredensial;
                         setKredensial(creds);
                    }
        ...
        <div className="flex-1 flex flex-col min-h-0 w-full max-w-full overflow-hidden bg-[var(--background)]">
            <header className="snappy-header w-full flex items-center justify-between gap-2 px-3 sm:px-4">
                <div className="flex items-center gap-1 min-w-0 flex-shrink-0">
                    ...
                    <span aria-live="polite" className="text-[10px] font-bold uppercase tracking-tight text-[var(--text-secondary)] opacity-50 truncate hidden sm:inline">
            ...
            <div className="flex-1 px-3 sm:px-4 py-2 overflow-y-auto no-scrollbar w-full">
                ...
                <div className="max-w-6xl mx-auto w-full">
                    {isCredentials && (
                        <PenyusunKredensial data={kredensial} onChange={(data) => setKredensial(data)} />
                    )}
                    <PenyusunCatatan
                        content={content}
                        onChange={(val) => {
