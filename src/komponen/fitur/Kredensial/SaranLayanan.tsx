...
export const SaranLayanan = ({ input, onSelect }: SaranLayananProps) => {
    ...
    const matches = Object.entries(MAP_LAYANAN)
        ...
        .slice(0, 8); // Show more matches for better selection
    ...
    return (
        <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
            className="absolute top-full left-0 right-0 z-[60] mt-1 glass-card overflow-hidden shadow-2xl border border-blue-500/30"
        >
            <div className="max-h-64 overflow-y-auto no-scrollbar py-1">
                ...
                {matches.map(([key, domain]) => (
                    <button
                        ...
                        className="w-full flex items-center gap-3 px-4 py-3 hover:bg-blue-500/10 text-left transition-colors active:bg-blue-500/20"
                    >
                        <div className="w-6 h-6 flex items-center justify-center flex-shrink-0">
                            {getIconForService(domain, 20)}
                        </div>
