                ...
                <>
                    <div className="absolute top-[-45px] right-0 flex items-center gap-2 z-10">
                        <button
                            ...
                            aria-label="Ganti ke font Monospace"
                        >
                            <Terminal size={16} />
                        </button>
                        <button
                            ...
                            aria-label="Ringkas catatan menggunakan AI"
                        >
                    ...
                    <BubbleMenu editor={editor} className="flex items-center gap-1 p-1 rounded-xl bg-[var(--background)] border border-[var(--separator)] shadow-lg backdrop-blur-md">
                        <button onClick={() => editor.chain().focus().toggleBold().run()} className={`p-2 rounded-lg hover:bg-[var(--glass-bg)] transition-colors ${editor.isActive('bold') ? 'text-[var(--primary)] bg-[var(--primary)]/10' : 'text-[var(--text-secondary)]'}`} aria-label="Tebalkan Teks">
                            <Bold size={16} />
                        </button>
                        <button onClick={() => editor.chain().focus().toggleItalic().run()} className={`p-2 rounded-lg hover:bg-[var(--glass-bg)] transition-colors ${editor.isActive('italic') ? 'text-[var(--primary)] bg-[var(--primary)]/10' : 'text-[var(--text-secondary)]'}`} aria-label="Miringkan Teks">
                            <Italic size={16} />
                        </button>
                    </BubbleMenu>
                    ...
                    <FloatingMenu editor={editor} className="flex items-center gap-1 p-1 rounded-xl bg-[var(--background)] border border-[var(--separator)] shadow-lg backdrop-blur-md">
                        <button onClick={() => editor.chain().focus().toggleHeading({ level: 1 }).run()} className={`p-2 rounded-lg hover:bg-[var(--glass-bg)] transition-colors ${editor.isActive('heading', { level: 1 }) ? 'text-[var(--primary)] bg-[var(--primary)]/10' : 'text-[var(--text-secondary)]'}`} aria-label="Judul 1">
                            <Heading1 size={16} />
                        </button>
                        <button onClick={() => editor.chain().focus().toggleBulletList().run()} className={`p-2 rounded-lg hover:bg-[var(--glass-bg)] transition-colors ${editor.isActive('bulletList') ? 'text-[var(--primary)] bg-[var(--primary)]/10' : 'text-[var(--text-secondary)]'}`} aria-label="Daftar Poin">
                            <List size={16} />
                        </button>
                        <button onClick={handleAISummary} className="p-2 rounded-lg hover:bg-[var(--glass-bg)] text-purple-500" aria-label="Ringkas AI">
                            <Sparkles size={16} />
                        </button>
                    </FloatingMenu>
                </>
