...
export const LayarKunciBrankas = () => {
    ...
    if (secretMode === 'gmail' && !isSetupMode) {
        return <PenyamaranGmail onUnlock={(pw) => handleUnlock(undefined, pw)} isLoading={isLoading} error={error} />;
    }
                    ...
                    >
                        ...
                        <form onSubmit={isSetupMode ? handleSetup : handleUnlock} className="w-full space-y-4">
                            <div className="ios-list-group shadow-sm">
                                <label htmlFor="vault-password" className="sr-only">Kata Sandi</label>
                                <input id="vault-password"
                                    type="password"
                                    value={password}
