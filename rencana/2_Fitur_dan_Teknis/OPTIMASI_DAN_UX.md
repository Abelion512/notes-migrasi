...
- **Lazy Loading Data:** Mengambil detail catatan dari IndexedDB hanya saat dibutuhkan. (Selesai via Incremental Loading)
- **Web Workers:** Memindahkan proses berat (seperti enkripsi/deskripsi massal) ke thread latar belakang agar UI tidak membeku.

## 2. Swipe Actions (iOS Style) - (Selesai)
**Tujuan:** Navigasi cepat dan intuitif.
- **Default Behavior:**
  - **Swipe Kanan:** Pin/Unpin (Warna Biru).
  - **Swipe Kiri:** Hapus (Warna Merah) & Arsip (Warna Oranye).
- **Kustomisasi:** User dapat mengatur aksi apa yang muncul pada setiap arah swipe di menu Setelan. (Segera)
